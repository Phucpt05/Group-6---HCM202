...
export const TimelineSection: React.FC = () => {
  ...
  const events = TIMELINE_SEGMENT_IDS.map((id) => HCM202_KNOWLEDGE_BASE.find((segment) => segment.id === id)).filter(Boolean);
      ...
      <div className="content-wrap">
        ...
        <div className="timeline">
          {events.map((event, index) => event && (
            <article className={`timeline-event reveal reveal-delay-${Math.min(index + 1, 3)}${reveal.isVisible ? " is-visible" : ""}`} key={event.id}>
              <div className="timeline-event__year">{labels[event.id]}</div>
